...
//    private static final String FILENAME = "src/censusIncome.csv";
    private static final String FILENAME = "src/winequality-red.csv";//"src/data/singlewine-reviews/winequality-red.csv";
...
//    private static int num_attributes = 11;
    ...
    private static int numberHiddenLayerNodes = 70;
    ...
    private static double[] temps = {1E8, 1E10, 1E12, 1E15, 1E18, 1E21};
    ...
    private static double best_temp = 1E20;
    private static double best_cooling = 0.7;
...
//    private static double[] mutateRatios = {0.02, 0.04, 0.06};
    ...
    private static double toMateRatio = 0.00;
...
//            Creates K-folds for cross-validation
        ...
        //    Determine best NN params, which happen to just be the shnumber of hidden layer nodes.
...
//    Find the index with the min avg validation error
     ...
     */
    public static void runBackprop(int trainIterations) throws IOException {
        ...
        File train_file = new File("src/results/backprop_train.csv");
        File test_file = new File("src/results/backprop_test.csv");
        BufferedWriter trainWriter;
        BufferedWriter testWriter;
        trainWriter = new BufferedWriter(new FileWriter(train_file));
        testWriter = new BufferedWriter(new FileWriter(test_file));
        ...
        for (int i = 0; i < trainIterations; i += 5) {
            ...
            trainWriter.write(Double.toString(trainError));
            testWriter.write(Double.toString(testError));
            testWriter.newLine();
            trainWriter.newLine();
...
//    Convert nanoseconds to seconds
     ...
     */
    public static void runRHC(int trainIterations) throws IOException {
...
//            System.out.println("== Iterations " + i + " ==");
        ...
        train(oa, backpropNet, trainIterations, "rhc");
...
//            System.out.printf("Test Error: %s%% %n", df.format(testError));
    ...
    public static void determineSAParams(int trainIterations) throws IOException {
        ...
        for (int x = 0; x < temps.length; x++) {
            ...
            for (int y = 0; y < coolingRates.length; y++) {
                ...
                for (int i = 0; i < nets.length; i++) {
                    ...
                    train(oas[i], nets[i], trainIterations, "null");
        ...
        for (int x = 0; x < temps.length; x++) {
            ...
            for (int y = 0; y < coolingRates.length; y++) {
//                System.out.println("Temp: " + Double.toString(temps[x]) + "  CR: " + Double.toString(coolingRates[y]) + "    Error: " + Double.toString());
                System.out.printf("Temp: %s%n CR: %s%n Error: %s%n", df.format(temps[x]), df.format(coolingRates[y]), df.format(errors[x][y]));
    ...
    public static void runSA(int trainIterations, double temp, double[] coolings) throws IOException {
        try {
...
//            OptimizationAlgorithm oa = new SimulatedAnnealing(1E15,
//                    coolingRates[j], nnop);
        OptimizationAlgorithm oa = new SimulatedAnnealing(1E18,
                best_cooling, nnop);

            train(oa, backpropNet, trainIterations * 7, "sim_anneal");
...
//        }
    ...
    public static void determineGAParams(int trainIterations) throws IOException {
            ...
            for (int y = 0; y < mateRatios.length; y++) {
                ...
                for (int z = 0; z < mutateRatios.length; z++) {
                    ...
                    for (int i = 0; i < nets.length; i++) {
                        ...
                        train(oas[i], backpropNet, trainIterations, "null");
...
//        System.out.printf("Error: %s%% %n", df.format(minErr));
     ...
     */
    public static void runGA(int trainIterations) throws IOException {
        ...
        for (int i = 0; i < mutateRatios.length; i++) {
            ...
            train(oa, backpropNet, trainIterations, "ga");
    ...
    private static void train(OptimizationAlgorithm oa, BackPropagationNetwork network, int
            iterations, String filename) throws IOException {
        train(oa, network, iterations, filename, false);
    }
    /**
     ...
     */
    private static void train(OptimizationAlgorithm oa, BackPropagationNetwork network, int
            iterations, String filename, boolean should_print) throws IOException {

        File train_file = new File("src/results/" + filename + "_train.csv");
        File test_file = new File("src/results/" + filename + "_test.csv");
        BufferedWriter trainWriter;
        BufferedWriter testWriter;
        trainWriter = new BufferedWriter(new FileWriter(train_file));
        testWriter = new BufferedWriter(new FileWriter(test_file));
        for(int i = 0; i < iterations; i++) {
            ...
            if (false || i % 5 == 0 || filename.equals("ga")) {
                trainWriter.write(Double.toString(trainError));
                testWriter.write(Double.toString(testError));
                testWriter.newLine();
                trainWriter.newLine();
