...
public class PokeAnalysisRunner {
	...
	public PokeAnalysisRunner(int MAX_ITERATIONS, int NUM_INTERVALS){
	...
	public void runValueIteration(PokemonWorld gen, Domain domain,
		...
		for(int numIterations = increment;numIterations<=MAX_ITERATIONS;numIterations+=increment ){
			...
			vi = new ValueIteration(domain, rf, tf, 0.99, hashingFactory, -1, numIterations); //Added a very high delta number in order to guarantee that value iteration occurs the max number of iterations
...
//		new EpisodeSequenceVisualizer(v, domain, Arrays.asList(ea));
		...
		PokePrinter.printPolicyMap(vi.getAllStates(), p, gen.startHealth);
		System.out.println("\n\n");
//		if(showPolicyMap){
...
//		}
	...
	public void runPolicyIteration(PokemonWorld gen, Domain domain,
			State initialState, RewardFunction rf, TerminalFunction tf, boolean showPolicyMap) {
...
//		uncomment
//		PokePrinter.printPolicyMap(vi.getAllStates(), p, gen.startHealth);
		PokePrinter.printPolicyMap(getAllStates(domain,rf,tf,initialState), p, gen.startHealth);
		System.out.println("\n\n");
...
//		}
	...
	public void runQLearning(PokemonWorld gen, Domain domain,
...
//		uncomment
//		PokePrinter.printPolicyMap(vi.getAllStates(), p, gen.startHealth);
		PokePrinter.printPolicyMap(getAllStates(domain,rf,tf,initialState), p, gen.getStartHealth());
		System.out.println("\n\n");
