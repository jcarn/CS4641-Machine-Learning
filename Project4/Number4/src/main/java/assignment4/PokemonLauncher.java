...
import assignment4.util.PokeAnalysisRunner;
import assignment4.util.PokeRewardFunction;
import assignment4.util.PokeTerminalFunction;
...
//		BasicGridWorld gen = new BasicGridWorld(map,maxX,maxY); //0 index map is 11X11
		PokemonWorld gen = new PokemonWorld(MAX_HEALTH); //0 index map is 11X11
		Domain domain = gen.generateDomain();

		State initialState = PokemonWorld.getExampleState(domain);

		RewardFunction rf = new PokeRewardFunction(MAX_HEALTH); //Goal is at the top right grid
		TerminalFunction tf = new PokeTerminalFunction(MAX_HEALTH); //Goal is at the top right grid

		SimulatedEnvironment env = new SimulatedEnvironment(domain, rf, tf,
				initialState);
		//Print the map that is being analyzed
		System.out.println("/////Poke World Analysis/////\n");
...
//		}
		...
		PokeAnalysisRunner runner = new PokeAnalysisRunner(MAX_ITERATIONS,NUM_INTERVALS);
